...
impl HPFile {
    pub fn new(
        ...
        Ok(HPFile {
            ...
            latest_file_size: AtomicI64::new(largest_file_size),
    ...
    fn parse_filename(segment_size: i64, file_name: &str) -> Result<i64> {
        ...
        if parts.len() != 2 {
            return Err(anyhow!(
                "{} does not match the patter 'FileId-segmentSize'",
                file_name
            ));
        }
    ...
    pub fn append(&self, bz: &[u8], buffer: &mut Vec<u8>) -> io::Result<i64> {
        ...
        let old_size = self
            .latest_file_size
            .fetch_and(bz.len() as i64, Ordering::SeqCst);
        ...
        buffer.extend_from_slice(&bz[split_pos..]);
